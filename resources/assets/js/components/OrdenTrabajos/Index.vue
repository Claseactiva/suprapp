<template>
    ...
    <div class="col-lg-12">
        ...
        <h5 class="text-white">
            Ordenes de trabajo
            <span class="badge badge-secondary ml-2">Total: {{ totalOrdenes }}</span>
            <span class="badge badge-warning ml-1">Incompletas: {{ ordenesIncompletas }}</span>
        </h5>
                    ...
                    <template v-for="ordentrabajoLocal in ordenestrabajos">
                        <tr data-toggle="collapse" :data-target="'#orden_trabajo_' + ordentrabajoLocal.id"
                            ...
                            <td>
                                ...
                                <span class="badge badge-warning" title="Trabajos incompletos">{{ (ordentrabajoLocal.trabajo || []).filter(trabajo => trabajo.realizado === 0).length }}</span>
...
export default {
    ...
    computed: {
        ...
        ordenesIncompletas() {
            return this.ordenestrabajos.filter(orden => (orden.trabajo || []).some(trabajo => trabajo.realizado === 0)).length
        },
        checkRealizado: {
            get() {
